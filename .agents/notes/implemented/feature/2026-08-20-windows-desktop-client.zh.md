...
桌面客户端与 `dsh web` 使用相同的 profile、凭据、会话、设置与 loopback HTTP 传输，因此插件不会获得仅适用于桌面的执行路径。分发包包含 Chromium 运行时，体积会大于 CLI。在发布基础设施提供代码签名前，打包需要通过网络访问固定版本的 Electron GitHub release，生成的 zip 也不会带有签名。
